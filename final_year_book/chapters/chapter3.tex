...
\subsubsection{III.5.3.1 Optimisation : URL Pré-signée pour l'Upload Direct}
Lors des tests d'intégration, un défi critique a été identifié : le transit des données binaires par la chaîne complète (Navigateur → Gateway → RabbitMQ → File Manager → S3) provoquait des erreurs de timeout pour les fichiers volumineux. La latence cumulée de chaque maillon du système distribué dépassait les délais d'attente configurés sur le broker de messages.

Pour résoudre ce problème de manière architecturalement correcte, nous avons adopté le pattern des \textbf{URL pré-signées} (\textit{Pre-signed URLs}) d'AWS S3. Ce pattern consiste à générer côté serveur une URL temporaire et sécurisée, permettant au client d'uploader un fichier \textbf{directement sur S3}, sans que les données binaires ne transitent par le backend.

\textbf{Nouveau flux d'upload :}
\begin{enumerate}
    \item Le client envoie une simple requête \texttt{GET /files/presigned-url} à la Gateway avec le nom et le type MIME du fichier.
    \item La Gateway transmet la demande au \textit{File Manager} via RabbitMQ. Ce dernier génère une URL signée valable 5 minutes et retourne \texttt{\{uploadUrl, finalUrl\}}.
    \item Le client upload le fichier \textbf{directement sur S3} via un \texttt{PUT} HTTP, sans passer par le backend.
    \item Le client envoie ensuite la requête de création du produit avec l'URL finale de l'image (\texttt{finalUrl}).
\end{enumerate}
...
async generatePresignedUploadUrl(
  ...
  // Génération de l'URL signée (expire en 5 minutes)
...
Ce refactoring illustre un avantage fondamental de l'architecture microservices : l'optimisation a été réalisée en modifiant \textbf{uniquement} le \textit{File Manager} et la Gateway, sans impacter aucun autre service. Le \textit{Product Manager}, l'\textit{Inventory Manager} et l'ensemble de l'écosystème restent inchangés.
...
\subsection{III.7.1 Construction des images : Multi-stage Building}
...
\subsection{III.7.2 Orchestration : Docker Compose}
...
Afin d'illustrer la dynamique de cette orchestration, la figure \ref{fig:docker_communication} schématise l'interaction en temps réel entre les différents conteneurs. L'API Gateway agit comme point d'entrée public, transférant les requêtes au réseau interne Docker où les microservices communiquent isolément via le broker RabbitMQ et accèdent à leurs bases de données respectives.

\begin{figure}[H]
    \centering
    \input{diagrams/docker_communication.tex}
    \caption{Visualisation de l'architecture des conteneurs Docker en cours d'exécution}
    \vspace{0.2cm}\small\textit{Source : Auteur}
    \label{fig:docker_communication}
\end{figure}


\subsection{III.7.3 Workflow de développement}
...
\section{III.8 Observabilité et Traçabilité Distribuée}
\label{sec:observability}

Pour concrétiser le pattern de traçabilité distribuée défini lors de la conception, nous avons intégré \textbf{Jaeger} et \textbf{OpenTelemetry}. Cette implémentation garantit une visibilité totale sur les requêtes asynchrones transitant par RabbitMQ.

\subsection{III.8.1 Instrumentation via la Shared Library}
Afin de ne pas dupliquer la configuration du tracing dans chaque microservice, l'initialisation a été centralisée au sein d'un fichier \texttt{tracing.ts} dans la bibliothèque partagée (\texttt{libs/shared}). Ce script configure le SDK OpenTelemetry pour intercepter automatiquement les requêtes HTTP, les requêtes aux bases de données et les échanges RabbitMQ.

\begin{verbatim}
// libs/shared/src/tracing.ts
...
Chaque microservice invoque cette fonction dans son fichier d'amorçage (\texttt{main.ts}), \textbf{avant} l'instanciation de l'application NestJS, garantissant ainsi que toutes les opérations sont tracées dès le démarrage du conteneur.

\section{III.9 Stratégie de Test et Validation}
\label{sec:testing}
...
\section{III.10 Visualisation de l'Interface Utilisateur (Frontend)}
Bien que ce mémoire se concentre sur la refonte du backend vers une architecture microservices, l'interface utilisateur reste le point de contact final. La plateforme Techcare se divise en deux environnements distincts : une interface client pour la vente en ligne et une interface administrateur dédiée à la gestion de l'officine.

\subsection{III.10.1 L'Espace Client (Interface de Vente)}
L'interface client est optimisée pour une navigation fluide et une expérience d'achat simplifiée. Elle permet aux patients et clients de consulter le catalogue des médicaments disponibles, de constituer un panier et de finaliser leurs commandes en toute sécurité. L'accès est sécurisé via un jeton JWT qui garantit la protection des données personnelles.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{diagrams/frontend_screenshot/ui_login.png}
    \caption{Interface de connexion et d'accès sécurisé (Client)}
    \vspace{0.2cm}\small\textit{Source : Auteur}
    \label{fig:ui_login_client}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{diagrams/frontend_screenshot/ui_catalogue.png}
    \caption{Interface de catalogue de médicaments (Client)}
    \vspace{0.2cm}\small\textit{Source : Auteur}
    \label{fig:ui_catalogue_client}
\end{figure}

\subsection{III.10.2 L'Espace Administrateur (Gestion Officinale)}
L'espace administrateur est l'outil de pilotage quotidien pour le pharmacien. Il regroupe les fonctionnalités critiques de gestion de stock, de suivi des ventes et de configuration de l'application en générale.

\subsubsection{III.10.2.1 Tableau de bord et indicateurs de performance}
Une fois authentifié, l'administrateur accède à une vue synthétique des activités. Les données (nombre de commandes du jour, alertes de stock critique) sont agrégées dynamiquement via des appels asynchrones à l'API Gateway, démontrant la réactivité de l'architecture distribuée.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{diagrams/frontend_screenshot/ui_dashboard.png}
    \caption{Tableau de bord administrateur : Statistiques et pilotage en temps réel}
    \vspace{0.2cm}\small\textit{Source : Auteur}
    \label{fig:ui_dashboard}
\end{figure}

\subsubsection{III.10.2.2 Gestion du catalogue et des inventaires}
Cette interface facilite la mise à jour du catalogue de médicaments. Chaque ajout de produit déclenche une orchestration complexe impliquant le service de gestion des fichiers (pour l'upload vers S3) et le microservice de catalogue (pour la persistance NoSQL), le tout de manière transparente pour l'utilisateur.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{diagrams/frontend_screenshot/ui_catalogue_admin.png}
    \caption{Interface de gestion des produits et mise en ligne du catalogue (Admin)}
    \vspace{0.2cm}\small\textit{Source : Auteur}
    \label{fig:ui_catalogue_admin}
\end{figure}
